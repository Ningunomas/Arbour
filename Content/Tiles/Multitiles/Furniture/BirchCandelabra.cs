...
internal class BirchCandelabra : ModTile
{
    ...
    public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
    {
        ...
        else if (tile.TileFrameX == 18 && tile.TileFrameY == 0)
        {
            offsetY = 6;
            offsetX = 4;
        }
        ...
        for (int c = 0; c < 7; c++)
        {
            ...
            float shakeY = Utils.RandomInt(ref seed, -10, 1) * 0.3f;
            ...
            Main.spriteBatch.Draw(flameTexture, pos + new Vector2(-3 + offsetX, 2), new Rectangle(0, 0, 16, 16), new Color(100, 100, 100, 0), 0f, default, 1f, SpriteEffects.None, 0f);
        }
        ...
        if (tile.TileFrameX == 0 && tile.TileFrameY == 0) //extra flame
        {
            for (int c = 0; c < 7; c++)
            {
                ...
                float shakeY = Utils.RandomInt(ref seed, -10, 1) * 0.3f;
                Vector2 pos = new Vector2(i * 16 - (int)Main.screenPosition.X - (width - 16f) / 2f + shakeX, j * 16 - (int)Main.screenPosition.Y + offsetY + shakeY) + zero;
                Main.spriteBatch.Draw(flameTexture, pos + new Vector2(4 + offsetX, -3), new Rectangle(0, 0, 16, 16), new Color(100, 100, 100, 0), 0f, default, 1f, SpriteEffects.None, 0f);
            }
        }
    }
}
